...
2. **Review the three-file system** architecture (types/*.d.ts, types/*.metadata.json, types/Tsonic.Node.bindings.json)
...
tests/Tsonic.Node.Tests/   # xUnit tests
├── PathTests.cs
├── FsTests.cs
...
1. Create C# implementation in `src/Tsonic.Node/<module>.cs`
2. Copy or create `.d.ts` file in `types/<module>.d.ts`
3. Add binding to `types/Tsonic.Node.bindings.json`
4. Add reference in `types/index.d.ts`
5. Create tests in `tests/Tsonic.Node.Tests/<Module>Tests.cs`
