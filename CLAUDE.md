...
3. Create metadata in `types/<module>.metadata.json`
4. Add binding to `types/Tsonic.NodeApi.bindings.json`
5. Add reference in `types/index.d.ts`
6. Create tests in `tests/Tsonic.NodeApi.Tests/<Module>Tests.cs`
...
# Update metadata if signatures changed
```
